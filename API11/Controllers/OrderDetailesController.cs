...
namespace API11.Controllers
{
    ...
    public class OrderDetailesController : ControllerBase
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateOrderDetailesAsync(int id, [FromBody] CreateOrderDetailesDto NewOrderDetailes)
        {
            var orderDetailes = await _context.OrdersDetailes.FindAsync(id);
            if (orderDetailes == null)
            {
        ...
        [HttpPost]
        public async Task<ActionResult<Category>> CreateOrderDetailesAsync(CreateOrderDetailesDto NewOrderDetailes)
        {
