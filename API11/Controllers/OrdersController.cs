...
namespace API11.Controllers
{
    ...
    public class OrdersController : ControllerBase
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateOrderAsync(int id, [FromBody] CreateOrderDto Neworder )
        {
            var order = await _context.Orders.FindAsync(id);
            if (order == null)
            {
        ...
        [HttpPost]
        public async Task<ActionResult<Category>> CreateOrderAsync(CreateOrderDto Neworder)
        {
            var order = new Order {
            Date = Neworder.Date,
